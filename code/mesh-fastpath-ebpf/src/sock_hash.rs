...
use aya_ebpf::{
    ...
    helpers::{
        ...
        bpf_sk_release, bpf_sock_hash_update, bpf_map_delete_elem,
    },
    //maps::PinningType,
...
impl<K> SockHash<K> {
    ...
    pub fn update(&self, mut key: K, sk_ops: *mut bpf_sock_ops, flags: u64) -> Result<(), i64> {
        let ret = unsafe {
    ...
    pub fn remove(&self, key: K) -> Result<(), i64> {
        let ret = unsafe {
            bpf_map_delete_elem(
                self.def.get() as *mut _,
    ...
    pub fn redirect_msg(&self, ctx: &SkMsgContext, mut key: K, flags: u64) -> i64 {
        unsafe {
    ...
    pub fn redirect_skb(&self, ctx: &SkBuffContext, mut key: K, flags: u64) -> i64 {
        unsafe {
