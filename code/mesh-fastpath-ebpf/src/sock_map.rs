...
use aya_ebpf::{
    ...
    helpers::{
        ...
        bpf_sk_release, bpf_sock_map_update, bpf_map_delete_elem,
    },
    //maps::PinningType,
...
impl SockMap {
    ...
    pub fn update(&self, mut index: u32, sk_ops: *mut bpf_sock_ops, flags: u64) -> Result<(), i64> {
        let ret = unsafe {
            bpf_sock_map_update(
                sk_ops,
    ...
    pub fn remove(&self, index: u32) -> Result<(), i64> {
        let ret = unsafe {
            bpf_map_delete_elem(
                self.def.get() as *mut _,
    ...
    pub fn redirect_msg(&self, ctx: &SkMsgContext, index: u32, flags: u64) -> i64 {
        unsafe {
    ...
    pub fn redirect_skb(&self, ctx: &SkBuffContext, index: u32, flags: u64) -> i64 {
        unsafe {
            bpf_sk_redirect_map(
                ctx.as_ptr() as *mut _,
                self.def.get() as *mut _,
